...
$.getJSON('/emoji/javascripts/emojilib/emojis.json', function(emojis) {
    ...
    Object.keys(emojis).forEach(function(key) {
        ...
        var charHTML
        charHTML =
            '<div class="js-emoji-char native-emoji" title="' +
            key +
            '">' +
            emoji['char'] +
            '</div>'
        container.append(
            '<li class="result emoji-wrapper js-emoji">' +
                charHTML +
                '<input type="text" class="js-emoji-code autofocus plain emoji-code" value=":' +
                key +
                ':" readonly /><span class="keywords">' +
                key +
                ' ' +
                emoji['keywords'] +
                '</span></li>'
    ...
    $('.emojis-container').toggleClass(
        'hide-text',
        localStorage.getItem('emoji-text-display') === 'false'
    )
})
...
$(document).on('click', '.js-hide-text', function() {
    ...
    var showorhide = $('.emojis-container').hasClass('hide-text')
        ? 'hide'
        : 'show'
    localStorage.setItem(
        'emoji-text-display',
        !$('.emojis-container').hasClass('hide-text')
    )
    return false
})
